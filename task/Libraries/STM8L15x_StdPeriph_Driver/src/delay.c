...
/* clear update flag */
  TIM4->SR1 &= ~TIM4_SR1_UIF; //SRI-status register, UIF - Update Interrupt Flag

/* Enable Counter */
  TIM4->CR1 |= TIM4_CR1_CEN; //Counter Enable Mask

  while(n_ms--)
  {
    TIM4->SR1=0u;
    while((TIM4->SR1 & TIM4_SR1_UIF) == 0)//while update event is not set, wait here
